...
> - **VN daily OHLC primary is now the DNSE EntradeX chart-api** (`dnse_client.py`, switched 2026-08-21 to kill SSI's ~10h daily-bar settlement lag that blanked same-day cells like DHC/DGW). Order is **DNSE → SSI FastConnect → vnstock**. SSI had been primary since the 2026-06-22 migration off vnstock `Trading.price_board()` (HTTP 403 from the cloud); it is now the second source. See `docs/AUDIT_FIXES_2026-08.md` §10 (DNSE) + [`docs/SSI_MIGRATION.md`](docs/SSI_MIGRATION.md) (SSI). **Scale differs by source:** DNSE stocks are thousand-VND as-is + indices ÷1000; SSI is raw-VND ÷1000. Any "vnstock `price_board` / `Trading`" references below are superseded (vnstock survives only as the final EOD fallback + `rs_source2` listing/fundamentals). Live intraday *price* (`intraday_breadth.py`) stays on SSI FastConnect — the DNSE switch is the **daily**-bar path only.
...
> - [`docs/AUDIT_FIXES_2026-08.md`](docs/AUDIT_FIXES_2026-08.md) — **2026-08 session handoff**: RS "recency" profile (option C), the data-sanity + logic-framework audit and its fixes (ingestion OHLC/scale guards, RS latest-rating fallback, intraday freshness guard, drop-stale-from-breadth, transactional publish), the new **`vn_trading_calendar.py`** (HOSE 2026 authoritative / 2027 provisional), the CI fix + master merge, the user's billing rework + killswitch footgun, and the taker_split prune. **Read this for the current code + deploy state.**
